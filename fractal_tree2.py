...
def change_series(value):
    global count_branch_series
    count_branch_series = value
    clear_curve()
    draw_graph()
...
def binary_tree2(len_branch, cnt):
    if cnt < count_branch_series:
        forward(len_branch)
        cnt += 1
        right(angle + skew)
        binary_tree2(len_branch, cnt)
        left(angle * 2.)
        binary_tree2(len_branch, cnt)
        right(angle - skew)
        backward(len_branch)
...
lbl_series = tk.Label(root, text="Branch series")
lbl_series.pack(side='left')
var_series = tk.StringVar(root)  # variable for spinbox-value
var_series.set(count_branch_series)  # Initial value
spn_series = tk.Spinbox(
    root, textvariable=var_series, from_=1, to=16, increment=1,
    command=lambda: change_series(int(var_series.get())), width=5
    )
spn_series.pack(side='left')
